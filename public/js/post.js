...
$(document).ready(function () {
    var positiveAudio = new Audio('./../../snd/positive-btn.mp3');
    var popAudio = new Audio('./../../snd/pop.mp3');
    ...
    //     // Send an AJAX request to post the content
    //     sendAjaxRequest(
    //         'post_create', // Use an appropriate operation like 'post_create'
        ...
        <div class="card newsfeed"  data-postid="${data.id}">
            <div class="newsfeed-body">
                <div class="author-box d-flex align-items-center justify-content-between">
                    <div class="author">
                        <img src="./../../img/profile/${data.user.avatar}" style="max-height: 40px;" alt="Profile" class="rounded-circle profile-sm">
                        <div class="info ms-2 mt-3">
                            <h5><a href="./../../../profile/${data.user.username}">${data.user.fullname}</a> ${displayCheck}</h5>
            ...
            <div class="card-footer">
                <div class="d-grid gap-2">
                    <div class="btn-group" role="group" aria-label="Basic outlined example">
                        <button type="button" class="btn btn-newsfeed heart-btn btn-sm"><i class="bi bi-heart me-1"></i> Love</button>
                        <button type="button" class="btn btn-newsfeed comment-btn btn-sm"><i class="bi bi-chat-square me-1"></i> Comment</button>
                    </div>
                </div>
    ...
    //     sendAjaxRequest('post_comment', requestData, function (response) {
    //         if (response.result) {
    //             var displayCheck = "";
    
    //             if (response.data.user.isOfficial) {
    //                 displayCheck += '<i class="ri ri-checkbox-circle-fill mt-2 text-danger" style="vertical-align: middle; margin-top: -1px;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Account is official."></i>';
    //             }
    
    //             if (response.data.user_id === response.data.post.user_id) {
    ...
    //             commentsCountElement.text(response.data.post.total_comments);
    
    //             // Create the new comment HTML
    //             const newComment = `
    //                 <li id="comment-${response.data.id}" class="comment-item" data-commentid="${response.data.id}"  data-username="${response.data.user.username}">
    //                     <div class="comment-content">
    //                         <img src="./../../img/profile/${response.data.user.avatar}" alt="" class="rounded-circle mt-3 profile-sm">
    //                         <div class="comment-message">
    //                             <div class="alert comment-box alert-dismissible" style="margin-bottom: 0px;">
    //                                 <h4><a href="./profile/${response.data.user.username}">${response.data.user.fullname}</a> ${displayCheck}</h4>
    //                                 <p class="prewrap">${response.data.content}</p>
    ...
    //                                     <a href="./../../../profile/${response.data.user.username}" tabindex="-1">
    //                                     <img src="./../../img/profile/${response.data.user.avatar}" alt="" class="rounded-circle mt-3 profile-sm">
    //                                     </a>
    //                                     <div class="input-group input-group-sm mt-2">
    //                                         <textarea class="form-control reply mt-1" style="resize: none; overflow-y: hidden; height: 40px;" spellcheck="false" placeholder="Reply to ${response.data.user.fullname}.."></textarea>
    ...
    //     sendAjaxRequest('post_reply', requestData, function (response) {
    //         if (response.result) {
    //             // Update the comments count in the HTML
    //             const replyCountElement = $('[data-postid="' + response.data.post.id + '"]').find('.comment-count');
    //             replyCountElement.text(response.data.total_comments);
    
    //             const isPostAuthor = response.data.user_id === response.data.post.user_id;
    ...
    //                         <div class="alert comment-box alert-dismissible" style="margin-bottom: 0px;">
    //                             <h4>
    //                                 <a href="./profile/${response.data.user.username}">${response.data.user.fullname}</a>
    //                                 ${authorBadgeHtml}
    //                             </h4>
    //                             <p class="prewrap text-wrap">${response.data.content}</p>
    //                             <button type="button" class="btn btn-light btn-sm btn-menu" data-bs-toggle="dropdown" aria-expanded="false">
    //                                 <i class="bi bi-three-dots"></i>
    //                                 <div class="dropdown-menu dropdown-menu-end" style="overflow: hidden;">
    ...
    //     const requestData = { post_id: postID, type: 'love' }; // Request data to send

    //     // Use the sendAjaxRequest function to send the AJAX request
    //     sendAjaxRequest('react_post', requestData, function (response) {
    //         // On success
    //         console.log('Post reacted:', postID);

    //         if (response.result === true) {
    //             if (response.reacting === true) {
    ...
    //                 $heartIcon.removeClass('bi-heart').addClass('bi-heart-fill'); // Change the heart icon to a filled heart
    //             } else {
    //                 $heartBtn.removeClass('text-danger'); // Remove the class if the user has un-reacted
    //                 $heartIcon.removeClass('bi-heart-fill').addClass('bi-heart'); // Change the heart icon back to an outline heart
    //             }
    //         } else {
    //             // On error or if response.result is false
    //             // Send toast to the user with error message
    ...
    //             } else {
    //                 $heartCount.text(response.data.count); // Update .heart-count with the response count
    //             }
    //         }

    //         // Send toast to the user with success message
    //         if (response.result === true) {
    //             createToast('Success', response.message, 'success');
    //         }
    //     }, function (error) {
    ...
    //         // Send AJAX request to react to the comment using sendAjaxRequest
    //         sendAjaxRequest(
    //             'react_comment', // Replace with your actual URL or operation name
    //             { comment_id: commentID, type: 'love' }, // Request data
    //             function (response) {
    //                 if (response.result === true) { // Check for true
    //                     // On success (Replace this with your actual success handling)
    //                     console.log('Comment reacted:', commentID);
    
    //                     // Update the "Love" button appearance and react count
    //                     if (response.reacting === true) { // Check for true
    ...
    //                     $reactCount.text(response.data.count);
    ...
    //                     }
    //                 } else {
    //                     // Handle the case where the comment no longer exists or another error occurred
    //                     // Send toast to user with response.message
    //                     createToast("Error", response.message, "danger");
    //                 }
    //             },
    //             function (response) {
    ...
    //         // Send AJAX request to react to the reply using sendAjaxRequest
    //         sendAjaxRequest(
    //             'react_reply', // Replace with your actual URL or operation names
    //             { reply_id: replyID, type: 'love'  }, // Request data
    //             function (response) {
    //                 if (response.result === true) { // Check for true
    //                     // On success (Replace this with your actual success handling)
    ...
    $('.newsfeed-posts').on('click', '.see-more-link', function(e) {
        e.preventDefault();
            
            const $shortenedContent = $(this).siblings('.shortened-content');
            const $fullContent = $(this).siblings('.full-content');

            $shortenedContent.toggleClass('visually-hidden');
            $fullContent.toggleClass('visually-hidden');
    ...
    $('.newsfeed-posts').on('click', '.comment-btn', function () {
        ...
        // Use sendAjaxRequest to send the AJAX request
        sendAjaxRequest('get_post', requestData, function (response) {
            if (response.result === true) {
                // Handle the success response here
                // You can access the post data from response.data
                const postData = response.data;
    
                // Populate the modal with data from postData
                $('#postModal .modal-title').text(postData.user.fullname + "'s Post");
               
                // Insert the post content into modal-body
                $('#postModal .modal-body').html(`
                    <div class="newsfeed" data-postid="${postData.id}">
                        <div class="newsfeed-body d-none d-lg-block">
                            <!-- Author information -->
                            <div class="author-box d-flex align-items-center justify-content-between">
                                <div class="author">
                                    <a href="https://cecilian-portal.online/profile/${postData.user.username}" tabindex="-1">
                                        <img src="https://cecilian-portal.online/img/profile/${postData.user.avatar}" style="max-height: 40px;" alt="Profile" class="rounded-circle profile-sm">
                                    </a>
                                    <div class="info ms-2 mt-3">
                                        <h5>
                                            <a href="https://cecilian-portal.online/profile/${postData.user.username}" tabindex="-1" class="fw-semibold name">${postData.user.fullname}</a>
                                            ${postData.user.isOfficial
                                                ? `<i class="bx bxs-badge-check text-danger" style="vertical-align: middle; margin-top: -1px;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Account is official."></i>`
                                                : ''
                                            }
                                            <a href="https://cecilian-portal.online/profile/${postData.user.username}" tabindex="-1">
                                                <small class="text-muted fw-lighter visually-hidden">@${postData.user.username}</small>
                                            </a>
                                        </h5>
                                        <p>
                                            <a href="https://cecilian-portal.online/${postData.user.username}/posts/${postData.post_hash_id}" tabindex="-1" class="text-secondary" data-bs-toggle="tooltip" data-bs-placement="bottom" tabindex="-1" data-bs-original-title="${postData.created_at}">${postData.formatted_created_at}</a>
                                        </p>
                                    </div>
                                </div>
                                <!-- Post menu dropdown -->
                                <div class="menu">
                                    <button type="button" class="btn text-secondary me-25 btn-sm reload-post">
                                        <i class="bi bi-arrow-counterclockwise"></i>
                                    </button>
                                    <button type="button" class="btn text-secondary btn-sm" data-bs-toggle="dropdown" aria-expanded="false">
                                        <i class="bi bi-three-dots-vertical"></i>
                                        <div class="dropdown-menu dropdown-menu-end" style="overflow: hidden;">
                                            <a class="dropdown-item edit-post" href="javascript:void(0);"><i class="bi bi-pencil-square me-1"></i> Edit post</a>
                                            <a class="dropdown-item disable-comment" href="javascript:void(0);"><i class="bi bi-toggle-off me-1"></i> Disable comment</a>
                            ...
                            <p class="prewrap text-break fw-light">${postData.content}</p>
                            <!-- Edit post section here -->
                            <div class="edit-post-section" style="display: none;">
                                <!-- Edit post form and buttons here -->
                            </div>
                        </div>
                        <div class="card-footer">
                            <div class="d-grid gap-2">
                                <div class="btn-group" role="group">
                                    <button type="button" class="btn btn-newsfeed heart-btn btn-sm ${postData.isReacted ? 'text-danger' : ''}">
                                        <i class="bi bi-heart${postData.isReacted ? '-fill' : ''} me-1"></i>
                                        <span class="heart-count">${postData.reactions_count === 0 ? 'Love' : postData.reactions_count}</span>
                                    </button>
                                    <button type="button" class="btn btn-newsfeed comment-btn btn-sm">
                                        <i class="bi bi-chat-square me-1"></i>
                                        <span class="comment-count">${postData.comments_count === 0 ? 'Comment' : postData.comments_count}</span>
                                    </button>
                                </div>
                            </div>
                            <div class="comment-section mt-2">
                                <ul class="user-comments list-inline mb-0">
                                    <!-- Comments and Replies go here -->
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                // Create a section for comments and replies
                const commentsSection = $('#postModal .modal-body .comment-section .user-comments');
    
                // Loop through the comments and add them to the comments section
                for (const comment of postData.comments) {
                    const commentElement = `
                        <li id="comment-${comment.id}" class="comment-item" data-commentid="${comment.id}" data-username="${comment.user.username}">
                        <div class="comment-content">
                        <a href="https://cecilian-portal.online/profile/${comment.user.username}" tabindex="-1">
                        <img src="https://cecilian-portal.online/img/profile/${comment.user.avatar}" alt="" class="rounded-circle mt-3 profile-sm">
                        </a>
                        <div class="comment-message">
                            <div class="alert comment-box alert-dismissible" style="margin-bottom: 0px;">
                                ${comment.user.isAuthor
                                    ? `<div class="d-flex align-items-center text-muted mb-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="This user is the author of the post." style="font-size: .625rem">
                                            <i class="ri-quill-pen-line me-1"></i>
                                            <span class="fw-light text-break">Author</span>
                                        </div>`
                                    : ''
                                }
                                <h4>
                                    <a href="https://cecilian-portal.online/profile/${comment.user.username}" tabindex="-1">Javeluna, James</a>
                                    ${comment.user.isOfficial
                                        ? `<i class="bx bxs-badge-check text-danger" style="vertical-align: middle; margin-top: -1px;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Account is official."></i>`
                                        : ''
                                    }
                                </h4>
                                <p class="prewrap text-break text-muted">${comment.content}</p>
                                <button type="button" class="btn text-secondary btn-sm btn-menu" data-bs-toggle="dropdown" aria-expanded="false">
                                    <i class="bi bi-three-dots"></i>
                                    <div class="dropdown-menu dropdown-menu-end" style="overflow: hidden;">
                                    ${postData.auth_id === comment.user.id
                                        ? `<a class="dropdown-item delete-comment" href="javascript:void(0);"> Delete</a>`
                                        : ''
                                    }
                                    <a class="dropdown-item report-comment" href="javascript:void(0);"> Report Comment</a>
                                    </div>
                                </button>
                            </div>
                            <p class="comment-meta">
                                <span class="text-left  heart-container" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-original-title="The total hearts received of this comment." style="font-size: 12px;display:none;">
                                <i class="bi bi-heart-fill text-danger"></i> 
                                <span class="react-count text-danger">${comment.reactionCount}</span>
                                </span>
                                <span class="ms-2 me-2 text-secondary love-btn ${comment.isReacted ? 'text-danger' : ''}">Love</span>
                                <span class="me-2 text-secondary reply-btn">Reply</span>
                                <a href="https://cecilian-portal.online/${comment.user.username}/posts/${postData.post_hash_id}?comment_id=${comment.id}" tabindex="-1" class="text-secondary">
                                    <span data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-original-title="${comment.created_at}">${comment.formatted_created_at}</span>
                                </a>
                            </p>
                        </div>
                        </div>
                        <ul class="comment-replies list-inline">
                        <li class="reply-item visually-hidden user-replying" data-commentid="${comment.id}/">
                            <div class="reply-content">
                                <a href="https://cecilian-portal.online/profile/${comment.user.username}" tabindex="-1">
                                <img src="https://cecilian-portal.online/img/profile/${comment.user.avatar}" alt="" class="rounded-circle mt-3 profile-sm">
                                </a>
                                <div class="input-group input-group-sm mt-2">
                                    <textarea class="form-control reply mt-1 mention-enabled" style="resize: none; overflow-y: hidden; height: 35px;" spellcheck="false" placeholder="Reply to ${comment.user.fullname}..."></textarea>
                                    <button type="button" class="btn btn-portal reply-submit mt-1" style="display:none;"><i class="ri-send-plane-fill me-1"></i></button>
                                </div>
                            </div>
                        </li>
                    ...
                    // Create a section for replies to this comment
                    const repliesSection = $('<ul class="comment-replies list-inline"></ul>');

                    // Loop through the replies and add them to the replies section
                    for (const reply of comment.replies) {
                        const replyElement = `
                            <li id="reply-${reply.id}" class="reply-item" data-replyid="${reply.id}" data-username="${reply.user.username}">
                                <div class="reply-content">
                                <a href="https://cecilian-portal.online/profile/${reply.user.username}">
                                <img src="https://cecilian-portal.online/img/profile/${reply.user.avatar}" alt="" class="rounded-circle mt-3 profile-sm">
                                </a>
                                <div class="comment-message">
                                    <div class="alert comment-box alert-dismissible" style="margin-bottom: 0px;">
                                        ${reply.user.isAuthor
                                        ...
                                        <h4>
                                            <a href="https://cecilian-portal.online/profile/${reply.user.username}" tabindex="-1">${reply.user.fullname}</a>
                                            ${reply.user.isOfficial
                                                ? `<i class="bx bxs-badge-check text-danger" style="vertical-align: middle; margin-top: -1px;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Account is official."></i>`
                                                : ''
                                            }
                                        </h4>
                                        <p class="prewrap text-wrap">${reply.content}</p>
                                        <button type="button" class="btn text-secondary btn-sm btn-menu" data-bs-toggle="dropdown" aria-expanded="false">
                                            <i class="bi bi-three-dots"></i>
                                            <div class="dropdown-menu dropdown-menu-end" style="overflow: hidden;">
                                            ${postData.auth_id === reply.user.id
                                                ? `<a class="dropdown-item delete-reply" href="javascript:void(0);"> Delete</a>`
                                                : ''
                                            }
                                            <a class="dropdown-item report-comment" href="javascript:void(0);"> Report Comment</a>
                                    ...
                                    <p class="comment-meta">
                                        <span class="text-left  heart-container" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-original-title="The total hearts received of this comment." style="font-size: 12px;display:none;">
                                        <i class="bi bi-heart-fill text-danger"></i> 
                                        <span class="react-count text-danger">${reply.reactionCount}</span>
                                        </span>
                                        <span class="ms-2 me-2 text-secondary love-btn ${reply.isReacted ? 'text-danger' : ''}">Love</span>
                                        <span class="me-2 text-secondary reply-btn">Reply</span>
                                        <a href="https://cecilian-portal.online/${reply.user.username}/posts/${postData.post_hash_id}?reply_comment_id=${reply.id}" tabindex="-1" class="text-secondary">
                                        <span data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-original-title="${reply.created_at}">${reply.formatted_created_at}</span>
                                        </a>
                                    </p>
                                </div>
                                </div>
                        ...
                        `;
                        // Append the reply element to the replies section
                        repliesSection.append(replyElement);
                    }
    
                    // Append the replies section to the corresponding comment
                    $(`#postModal .comment-item[data-commentid="${comment.id}"]`).append(repliesSection);
                }
    ...
    $('.newsfeed-posts').on('click', '.reply-btn', function () {
        ...
        if (currentText === '') {
            // If the textarea is empty, add the @mention without a space
            textarea.val('@' + commentUsername + ' ');
        } else if (currentText.indexOf('@' + commentUsername) === -1) {
            // If username is not in the textarea, add it with a space
    ...
    $('.newsfeed-posts').on('click', 'a.delete-post', function (event) {
        ...
        // Send the AJAX request to delete the post using the sendAjaxRequest function
        sendAjaxRequest('post_delete', { post_id: postId }, function (response) {
        // Check the response.result property
        if (response.result === true) {
            ...
            createToast('Error', response.message, 'error');
        }
        }, function (error) {
        createToast("Error", error.message, "danger");
    ...
    //                 <img src="./img/svg/water.jpg" class="img-fluid mb-5" alt="Page Not Found">
    //             `;
